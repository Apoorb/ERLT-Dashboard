...
title_1 = html.H3("MOVES 2014b Daily Composite Extended Idling Emission Rates")
...
layout = dbc.Container(
    [
        ...
        dbc.Row([dbc.Col(dropdown_2, width=3)]),
        dbc.Row(
            dbc.Card(
                html.H4(
                    "MOVES 2014b Extended Idling Emission Rates Plots",
        ...
        dbc.Row(
            dbc.Col(
                dcc.Graph(
                    id="extidle_erlt_line", style={"width": "100%", "height": "600px"}
                ),
                className="col-md-8",
            )
        ),
        dbc.Row(
            dbc.Card(
                html.H4(
                    "MOVES 2014b Extended Idling Emission Rates Table",
                    className="text-center text-light bg-dark",
                ),
                body=True,
                style={"padding": "6px"},
        ...
        dbc.Row(
            dash_table.DataTable(
                id="datatable_interactivity_extidle",
                columns=[
...
@app.callback(
    Output("datatable_interactivity_extidle", "data"),
    [Input("pollutant-dropdown", "value")],
)
def update_table(pol_val):
    erlt_df_area_pol = erlt_df_long_fil.loc[(erlt_df_long_fil.Pollutant == pol_val)]
    return erlt_df_area_pol.to_dict("records")


@app.callback(
    Output("extidle_erlt_line", "figure"), [Input("pollutant-dropdown", "value")]
)
def update_extidle_erlt_line(pol_val):
